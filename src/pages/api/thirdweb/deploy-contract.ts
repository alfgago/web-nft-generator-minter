...
export default async function handler(
  ...
  try {
    const { network, wallet, name } = req.body
    ...
    const totalRoyaltyBlips = 100
    const splitBips =
      process.env.ADMIN_WALLET_ADDRESS != wallet
        ? totalRoyaltyBlips / 2
        : totalRoyaltyBlips

    const paymentSplits = [
      {
        address: process.env.ADMIN_WALLET_ADDRESS, // Admin wallet
        sharesBps: splitBips,
      },
    ]

    if (wallet) {
      if (process.env.ADMIN_WALLET_ADDRESS != wallet) {
        paymentSplits.push({
          address: wallet, // Artist wallet
          sharesBps: splitBips,
        })
      }
    }
